...
export default function BlogCard({ blog }: { blog: BlogI }) {
  return (
    <Link
      href={`/blog/${blog.slug}`}
      className="mt-12 grid grid-cols-3 rounded-lg bg-[#615E5E] text-[#D8D6D6] p-6"
    >
      <div className="col-span-2">
        ...
        <div>
          ...
          <p className="text-[#F2F2F2] mt-2 text-sm">{shortify(blog.description, 100)}</p>
        </div>
        <p className="text-sm mt-8">
          <span className="text-white bg-[#8A8A8A] py-[1px] px-3 rounded-full mr-2">{blog.tags[0]}</span> {blog.readingTime} min read
        </p>
      </div>
